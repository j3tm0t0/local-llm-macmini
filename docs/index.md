...
Mac mini を **縦置きにして上に USB クーラーを載せて** active cooling を効かせて再走行したところ、**MLX vllm-mlx + Qwen3-Coder DWQ の T5 verbose loop (10 分タイムアウト) が消えて累計 22m36s → 13m12s (-42%)**。Ollama gpt-oss:20b も -15%、軽いモデルでも -7%。Apple 純正の冷却機構は LLM 連続生成を想定していないらしい。
