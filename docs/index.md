...
- **冷却ファンは効く (構成依存だが劇的な場合あり)**: Mac mini 背面に外付けファンを置くだけで Ollama gpt-oss:20b は -15%、**MLX vllm-mlx + Qwen3-Coder DWQ は累計 22m36s → 13m12s (-42%) で T5 timeout も解消**。ただし robustonian fork は VRAM 圧で逆効果 (+5%)。詳細は[冷却検証セクション](#冷却ファン投入で-thermal-throttling-を抑えるとどうなる)。
...
4. **連続生成時のサーマルスロットリング**: M4 Pro は GPU が 10 秒で 92°C に達してスロットリング入りする。比較ベンチを取るなら **テスト間に 30〜60 秒のクールダウン**を挟むこと。MLX 版テストスクリプトでは 45s 固定で挿入している。**外付けファンを後付けするとさらに改善する** — 詳細は次セクション。

---

## 冷却ファン投入で thermal throttling を抑えるとどうなる？

検証当日 GPU が 92-95°C に張り付いていたので、Mac mini M4 Pro 背面に外付けの静音ファンを置いて active cooling を効かせ、同じ T2-T5 を再走行した。
...
**Mac mini M4 Pro でローカル LLM を回すなら、背面に静音ファンを 1 台置くのは強くおすすめ**。特に MLX 系の重いモデル / 長尺タスクで詰まるケースは劇的に改善する場合がある (構成依存)。Apple 純正の冷却機構は LLM 連続生成を想定していない。
