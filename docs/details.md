...
検証当日 GPU が 92-95°C に張り付いていたので、**Mac mini M4 Pro を縦置きにして、上に USB クーラー (角型ファン) を載せて** active cooling を効かせ、同じ T2-T5 を再走行した。

![Mac mini を縦置きにして上に USB クーラーを載せた設置](assets/macmini-with-cooler.jpg)

> 写真の構成: Mac mini M4 Pro を縦置き (Apple ロゴが正面)、本体上面 (= 元の側面) にちょうど被さるサイズの USB 給電 12cm ファンを直接載せて、本体に風を吹き付けている。
...
**Mac mini M4 Pro でローカル LLM を回すなら、縦置き + 上から USB クーラーで風を当てるのは強くおすすめ**。特に MLX 系の重いモデル / 長尺タスクで詰まるケースは劇的に改善する場合がある (構成依存)。Apple 純正の冷却機構は LLM 連続生成を想定していない。

> **置き方メモ**: 最終的には写真のとおり、**Mac mini を縦に立てて上面に USB 給電の 12cm ファンを直接載せる** スタイルに落ち着いた。本体側面 (元の天面) がちょうどファンの吸気/排気面に被さるので筐体ヒートシンク全体に風が通る。横から当てる方式も試したが、固定が面倒だった割に上載せ方式とほぼ同じ効果だったので、台座を使わず重力で済むこの形が運用上ラク。
